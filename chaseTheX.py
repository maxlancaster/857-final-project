...
def chaseTheX(chaser, balls, strikes, outs):
	while True:
		sequence = []
		for i in range(5):
			sequence.append(random.randint(1,5))
		if chaser in sequence:
			break
	sequence.append(balls)
	sequence.append(strikes)
	sequence.append(outs)
	findthechaser = sequence.index(chaser)
	actualPitch = sequence[findthechaser+1]
...
def generateData(atBats):
	...
	for i in range(atBats):
		...
		while count[0]!=4 and count[1]!=3:
			chase=chaseTheX(chaser, count[0], count[1], outs)
			data.append(chase[0])
...
ax.set_title("Neural Network Learning/Decoding of Chase the 2 Scheme")
# 179 at-bats needed
plt.show()